...
# Upgrade schema OpenMSP DB from 1.0 (db_example.sqlite3) to 1.1 (db.sqlite3 structure).
# Usage:
#   ./static/update/update_1_0_to_1_1.sh [path/to/target.sqlite3] [path/to/reference_v1_1.sqlite3] [path/to/target.env] [path/to/reference_v1_1.env] [path/to/reference_v1_0.env_example]

DB_PATH="${1:-db.sqlite3}"
REF_DB_PATH="${2:-db.sqlite3}"
ENV_TARGET_PATH="${3:-.env}"
ENV_REF_V11_PATH="${4:-.env}"
ENV_REF_V10_PATH="${5:-.env_example}"
...
if [[ ! -f "$REF_DB_PATH" ]]; then
  echo "Errore: database di riferimento 1.1 non trovato: $REF_DB_PATH"
  exit 1
fi
...
if [[ ! -f "$ENV_REF_V11_PATH" ]]; then
  echo "Errore: file .env di riferimento 1.1 non trovato: $ENV_REF_V11_PATH"
  exit 1
fi

if [[ ! -f "$ENV_REF_V10_PATH" ]]; then
  echo "Errore: file .env_example di riferimento 1.0 non trovato: $ENV_REF_V10_PATH"
  exit 1
fi
...
parse_env_to_map "$ENV_REF_V10_PATH" env_v10

for key in "${!env_target[@]}"; do
  if [[ -v env_v10["$key"] && -v env_v11["$key"] ]]; then
    if [[ "${env_target[$key]}" == "${env_v10[$key]}" && "${env_v10[$key]}" != "${env_v11[$key]}" ]]; then
      env_replacements["$key"]="${env_v11[$key]}"
    fi
  fi
done
